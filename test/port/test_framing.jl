# Phase 1c port of test/test_framing.jl to the merged API.
#
# The original csvance test targets the framing layer that the merge ported
# almost verbatim (grpc_encode_message_iobuffer, FrameReader, read_message!,
# expect_half_close!, GRPC_HEADER_SIZE all live in the merged module). The
# adaptations: gRPCServiceCallException -> GRPCError (asserting the status code
# too), TestServiceServer.TestResponse comes from the shared Phase 1c harness (gRPCClient v1.1.0's
# generated types), and the legacy-only _decode_message is replaced by the
# merged deserialize_message(io, "Vector{UInt8}"; raw=true) with the same fresh
# copy semantics.
#
# The original stays untouched at test/test_framing.jl for the Phase 3 verbatim
# compat gate.
...
@testset "Framing" begin
    ...
    msg = TestServiceServer.TestResponse(collect(UInt64, 1:7))
    ...
    roundtrip = decode(ProtoDecoder(IOBuffer(payload)), TestServiceServer.TestResponse)
    @test roundtrip.data == msg.data

    # Empty message is valid: a 5-byte frame with length 0.
    empty_bytes = take!(grpc_encode_message_iobuffer(TestServiceServer.TestResponse(UInt64[])))
    ...
    big_msg = TestServiceServer.TestResponse(collect(UInt64, 1:1000))
    ...
    msgs = [
        TestServiceServer.TestResponse(collect(UInt64, 1:7)),
        TestServiceServer.TestResponse(UInt64[]),                  # empty frame (length 0)
        TestServiceServer.TestResponse(collect(UInt64, 100:140)),
        TestServiceServer.TestResponse(zeros(UInt64, 80_000)),     # > 64 KiB initial buffer
        TestServiceServer.TestResponse(collect(UInt64, 1:3)),
    ]
    ...
    for m in msgs
        io = read_message!(fr)
        @test io !== nothing
        @test decode(ProtoDecoder(io), TestServiceServer.TestResponse).data == m.data
    end
    ...
    oversize_prefix_io() = begin
        io = IOBuffer()
        write(io, take!(grpc_encode_message_iobuffer(TestServiceServer.TestResponse(collect(UInt64, 1:1000)))))
        seekstart(io)
        io
    end
    ...
    full = take!(grpc_encode_message_iobuffer(TestServiceServer.TestResponse(collect(UInt64, 1:50))))
    ...
    extra_frame_io() = begin
        io = IOBuffer()
        write(io, take!(grpc_encode_message_iobuffer(TestServiceServer.TestResponse(collect(UInt64, 1:3)))))
        seekstart(io)
        io
    end
    ...
    raw_payload =
        take!(grpc_encode_message_iobuffer(TestServiceServer.TestResponse(collect(UInt64, 1:9))))[(GRPC_HEADER_SIZE+1):end]
